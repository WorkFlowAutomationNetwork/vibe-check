...
*2026-06-17 (scanner) — **Rate-limit discovery fix + honest active-scan reporting.** A real deep scan of bathroomhealthos.com exposed a trust bug: the site has a genuine login form at `/portal/login`, but `RateLimitScanner` missed it entirely and silently returned `[]`, while the report UI still showed a green "✓ Authentication Review" badge as if the check had run clean. Two compounding root causes: (1) discovery only fetched the homepage plus a fixed list of guessed paths — it never followed real nav links, so non-standard login paths were invisible; (2) the form-discovery regex required a captured `action="..."` attribute, but the form was a React/Next.js `<form>` with no action (submits via `onSubmit`/`fetch()`), so even a found login page yielded no testable endpoint. **Fix (full brainstorm→spec→plan→subagent-driven-development cycle, spec at `docs/superpowers/specs/2026-06-17-rate-limit-discovery-and-honest-reporting-design.md`):** `rate_limit.py` redesigned to follow up to 2 login-ish homepage links and fall back to the login page's own URL when a form has no action, all within a bounded budget (17 requests worst case: 1 homepage GET + 2 link GETs + 6 generic-path GETs + the 8-POST battery run once on a single chosen target). Replaced every silent `return []` across the three active-tier scanners with explicit `severity="info"` findings — `RateLimitScanner` (no target found / found-but-inconclusive), `SupabaseExposureScanner` (2 cases), `StorageExposureScanner` (3 cases) — so a check that found nothing to test now says so instead of reporting nothing. `StackUpgradeBlock.tsx` now renders three badge states from the real findings: locked (didn't run / free tier), neutral grey (ran, nothing applicable to test, shows the info finding's note), green check (ran, found an actionable result) — no more static checkmark. Scanner suite: 142 passing; web type-check clean. **Note:** this only affects scans run after deployment — it does not backfill already-completed scans (including the bathroomhealthos.com scan that started this). Requires a Fly.io redeploy of the scanner service to take effect in production, per the redeploy pattern below.*

*2026-06-17 (stripe) — **Stripe sandbox wired up.** Created test-mode products/prices (`Vibe-Check Starter` $9 one-off, `Vibe-Check Monitor` $19/mo, looked up by `lookup_key` not hardcoded ID) via the Stripe API using `apps/web/.env.local`'s `sk_test_...` key. New `apps/web/app/api/billing/checkout/route.ts` creates a Checkout Session (`mode: payment` for Starter, `mode: subscription` for Monitor) and is now what the billing page's upgrade buttons link to (previously linked to the customer-portal route, which only works for existing Stripe customers — a dead end for anyone upgrading from Free). `stripe-webhook/route.ts`'s `checkout.session.completed` handler now also sets `profiles.plan` for one-time payments (Starter never fires `customer.subscription.created`, so without this the plan column would never update for one-off purchases). Added missing `NEXT_PUBLIC_APP_URL` to `.env.example` (was referenced in 4 files, undocumented — known issue, now closed). Installed Stripe CLI (`winget install --id Stripe.StripeCli -e`) and got a stable test-mode webhook signing secret via `stripe listen --api-key <key> --print-secret`; documented the local webhook-forwarding command in `CLAUDE.md`'s Development workflow section. **Caught a near-miss:** the Stripe MCP connector tool available in this environment is wired to the **live** Stripe account with no test-mode switch — it silently created two live products before this was noticed (`livemode: true` in the response). Both were archived (`active: false`, no prices ever attached, so no real exposure) and rebuilt correctly via direct test-mode API calls instead. **Update — verified end-to-end, one real bug found and fixed.** User ran a real test purchase ($9 Starter) through the actual browser flow; Stripe processed it but the resulting Checkout Session had `customer: null`, so the webhook's `if (session.customer && userId)` guard silently no-op'd — `profiles.plan`/`stripe_customer_id` never updated despite Stripe-side success. Root cause: payment-mode Checkout Sessions only create a Customer object when `customer_creation: 'always'` is explicitly set (subscription mode always creates one; payment mode defaults to not). Fixed in `checkout/route.ts` — `customer_creation: 'always'` now set whenever a Starter (`mode: payment`) session is created for a user with no existing `stripe_customer_id`. Verified the fix by replaying a faithful `checkout.session.completed` payload (real-shape, HMAC-signed with the actual `STRIPE_WEBHOOK_SECRET`) against the running webhook route: confirmed via direct Supabase query that `profiles.plan` → `'starter'` and `stripe_customer_id` got set correctly. Also confirmed via `plan_limits` table that "Starter" is already modeled as a **persistent plan unlock** (`max_scans_per_month: null`, `allowed_scan_types: [passive, active]`), not a single-use scan credit — the $9 one-off payment is a lifetime-style unlock by existing design, not a bug; no scan-credit system needs building.

**Update 2 — Monitor subscription flow (upgrade + downgrade) verified end-to-end.** Found and fixed a second gap before testing: **no Stripe Customer Portal configuration existed in test mode** — `stripe.billingPortal.sessions.create()` would have failed outright for any "Manage subscription" / downgrade click. Created one via the API (`invoice_history`, `payment_method_update`, `customer_update[email]`, `subscription_cancel` at period end all enabled; `is_default: true`), confirmed a portal session now creates successfully. Then drove a full real subscription lifecycle through the actual Stripe API (test card token → attached PaymentMethod → `subscriptions.create` with the `monitor_monthly` price) with `stripe listen` forwarding to the running dev server: `customer.subscription.created` → `profiles.plan` correctly became `'monitor'` with `stripe_subscription_id` set; `subscriptions.cancel` → `customer.subscription.deleted` → `profiles.plan` correctly reverted to `'free'`, `stripe_subscription_id` cleared. Both legs went through unmodified webhook code — no further backend fix needed here, just the missing portal config. Test customer (`cus_UiglqId4B98LJv`), payment method, and subscription all exist in **test mode only**, no cleanup needed.*
